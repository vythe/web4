...
public class BotHallAPI
{
	...
	public BHSession.UpdateBin getUpdate() 
	{
		...
		agent.timecode = s.engine.timecode;
...
*/		
	...
	public String joinSession(@QueryParam("id") String sessionID)
	{
		...
		if (agent == null)
		{
			agent = BHClientAgent.createAgent(s);
		}
	...
	public String joinMobile(@QueryParam("name") String name)
	{
		...
		else
		{
			...
			for (BHCollection.Atom a : s.engine.getCollection().all())
			{
	...
	public String cycleMode(@QueryParam("run") String run)
	{
		...
		if (s == null || s.engine == null || run == null)
		{
			return "0";
		}
		if ("Y".equals(run) && !s.engine.isRunning)
		{
			s.engine.startCycling();
		}
		else if ("N".equals(run) && s.engine.isRunning)
		{
			s.engine.stopCycling();
		}
		else if ("O".equals(run) & !s.engine.isRunning)
		{
			BHAction stopAction = new BHAction();
			stopAction.actionType  = BHOperations.ACTION_STOPCYCLING;
			
			s.engine.postAction(stopAction, 0);
			s.engine.startCycling();
		}
		return "" + s.engine.timecode;
	}
}
